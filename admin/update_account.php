<?php
	...
	if ($cpassword!=$apassword){
		?>
		<script>
			window.alert('Contraseña de verifiacion incorrecta!');
			window.history.back();
		</script>
		<?php
	}
	
	elseif ($cpassword!=$myqrow['Contraseña']){
		?>
		<script>
			window.alert('La contraseña insertada no es la misma!');
			window.history.back();
		</script>
		<?php
	}
		...
		if ($mpassword==$myqrow['Contraseñá']){
